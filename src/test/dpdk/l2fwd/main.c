...
l2fwd_main_loop(void)
{
	...
	/* Print qconf details */
	RTE_LOG(INFO, L2FWD, "qconf details for lcore %u:\n", lcore_id);
	RTE_LOG(INFO, L2FWD, "  Number of RX ports: %u\n", qconf->n_rx_port);
	RTE_LOG(INFO, L2FWD, "  RX port list:");
	for (i = 0; i < qconf->n_rx_port; i++) {
		RTE_LOG(INFO, L2FWD, " %u", qconf->rx_port_list[i]);
	}
	RTE_LOG(INFO, L2FWD, "\n");
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				...
				// Extract and log the IPv4 header values
				if (eth_hdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
					ip_hdr = (struct rte_ipv4_hdr *)(eth_hdr + 1);
					// RTE_LOG(INFO, L2FWD, "IPv4 Header - Source IP: %u.%u.%u.%u, Protocol: %u\n",
					...
					if (ip_hdr->next_proto_id == IPPROTO_ICMP ) {
						RTE_LOG(INFO, L2FWD, "ICMP packet received on port %d\n", portid);
